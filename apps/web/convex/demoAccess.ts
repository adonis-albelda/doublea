...
// Fetches a per-user demo login (admin email + password, cashier PIN) from
// the Tally superadmin API — the URL and API key are Convex env vars
...
export const fetchAccessCode = action({
  ...
  handler: async (
    ctx,
  ): Promise<{ demoEmail: string; password: string; cashierPin: string; validForDate: string | null }> => {
    const userId = await getAuthUserId(ctx);
    ...
    const body: {
      data?: { demo_email?: string; password?: string; cashier_pin?: string; valid_for_date?: string };
    } = await response.json();
    const demoEmail = body.data?.demo_email;
    const password = body.data?.password;
    const cashierPin = body.data?.cashier_pin;
    ...
    if (!cashierPin) throw new Error("Access code response was missing `data.cashier_pin`.");

    return { demoEmail, password, cashierPin, validForDate: body.data?.valid_for_date ?? null };
  },
});
